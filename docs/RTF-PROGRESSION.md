...
The personal four-day exercise lineup and existing working-set counts are retained instead of the workbook's sample five sets per lift. Bench and the primary shoulder press use the main schedule; selected presses and rows use auxiliary progression. Leg extension and other accessories use last-set rep-out progression: work sets at the bottom of the rep range, rep out on the final set with the top of the range as the target, and reach it on a complete, unmodified, pain-free session to add one equipment increment next session. Anything else holds the load; accessories never decrease. RDL, calves and other controlled-profile work use the same rep-out progression. Accessories/controlled work deload to half sets rounded up and 90% load rounded to equipment, while SBS lifts retain their set counts as in the sheet.

Starting training maxima are explicit estimates from the user's reported performance, never synthetic logged sets. Unknown RTF loads calibrate from the first working weight divided by the scheduled intensity. Subsequent sets keep that first load. The final rep-out result then adjusts the inferred max if all qualifying sets were completed. When every working set is logged at one load heavier than prescribed, the training max is re-anchored from the actual load (actual ÷ intensity) instead of held; lighter or mixed loads still hold. No estimated barbell-to-dumbbell conversion is used.

An optional `trainingMaxOverride` on a main/auxiliary lift and `loadOverride` on an accessory raise the floor for future sessions; earned progression continues above the override, and overrides below the earned value stay dormant.
